...
if output_path.suffix == ".h5ad":
    ...
    raise ValueError(
        f"Unknown file format: {output_path.suffix}. Supported formats are .h5ad and .csv."
    )
...
config = {
    ...
    "return_transcript_data": snakemake.params["counts_from_abundance"]
    not in ["None", "length_scaled_tpm"],
    "counts_from_abundance": None
    if snakemake.params["counts_from_abundance"] == "None"
    else snakemake.params["counts_from_abundance"],
    ...
    if snakemake.params["inferential_replicates"]
    else None,
