...
sns.scatterplot(
    x=pca_counts[:, 0],
    y=pca_counts[:, 1],
    hue=ad_counts.obs["condition"].values,
    ax=ax1,
)
...
if snakemake.params["min_max_counts"] > 0:
    ad_counts = ad_counts[
        :, ad_counts.X.max(axis=0) >= snakemake.params["min_max_counts"]
    ].copy()

if snakemake.params["min_mean_counts"] > 0:
    ad_counts = ad_counts[
        :, ad_counts.X.mean(axis=0) >= snakemake.params["min_mean_counts"]
    ].copy()
...
if snakemake.params["min_sample_counts"]:
    ad_counts = ad_counts[
        ...
        (
            np.sum(
                ad_counts.X >= snakemake.params["min_sample_counts"]["min_counts"],
                axis=0,
            )
...
if snakemake.params["shrink_log2_fold_change"]:
    ds.lfc_shrink(coeff=f"condition[T.{snakemake.params['treated_name']}]")

df_results = ds.results_df
...
if "gene_name" in df_transcript_to_gene.columns:
    gene_id_to_gene_name = (
        df_transcript_to_gene.set_index("gene_id")["gene_name"]
        .drop_duplicates()
        .to_dict()
    )
    df_results["gene_name"] = (
        df_results.index.to_series().map(gene_id_to_gene_name).values
    )
